...
The Thunderbird team has finished the conversion of all its JSM files and now only ES6 modules instead. There is still a compatibility layer, which maps requests for the old `*.jsm` files to the new `*.sys.mjs` files. It is however recommended to use the new files now already. The new files are either loaded via `ChromeUtils.importESModule()`:
