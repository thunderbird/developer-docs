...
As noted in the [Setting up a build environment page](../setting-up-a-build-environment.md), both Firefox and Thunderbird sources are stored in Git repositories. This means you will need to install Git if it is not already available. Here are some quick commands to install on common distributions:
